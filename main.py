...
def get_geolocation():
    try:
        ...
        data = response.json()
        return data[
            'loc']  # Returns the latitude and longitude as a string "lat,lng"
...
def book_product(user_id, product_id):
    ...
    )  # Get the geolocation when booking the product
    conn = sqlitecloud.connect(
        "sqlitecloud://ce3yvllesk.sqlite.cloud:8860/gas?apikey=kOt8yvfwRbBFka2FXT1Q1ybJKaDEtzTya3SWEGzFbvE"
    )
    cursor = conn.cursor()
    cursor.execute(
        'INSERT INTO bookings (user_id, product_id, status, geolocation) VALUES (?, ?, ?, ?)',
        (user_id, product_id, 'Processing', geolocation))
    conn.commit()
    conn.close()
...
def fetch_bookings(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT product_id, status FROM bookings WHERE user_id = ?',
                   (user_id, ))
    ...
    for product in products:
        ...
        def on_book(e, product_id=product_id, product_name=product_name):
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text(f"Product {product_name} booked successfully!"),
                open=True)
        ...
        product_details.append(
            ft.ElevatedButton(text="Book Now",
                              on_click=on_book,
        ...
        product_container = ft.Container(content=ft.Card(
            content=ft.Column(
                product_details,
...
def main(page: ft.Page):
    ...
    page.appbar = ft.AppBar(title=ft.Text("GAS BOOKING"),
                            center_title=True,
                            bgcolor="#000000")
    ...
    def on_signin(e):
        ...
        if user:
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text("Invalid username or password"), open=True)
    ...
    def on_signup(e):
        ...
        page.snack_bar = ft.SnackBar(
            ft.Text("Signup successful! Please sign in."), open=True)
        page.update()  # Ensure the page is updated
